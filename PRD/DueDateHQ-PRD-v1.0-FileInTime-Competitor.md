...
| --- | --- | --- |
| 内容护城河 | 官方公告抓取需要持续人肉兜底（周末/节假日/格式变化） | 14 天 MVP 先做 5 州 + IRS，以后每增加 1 州就多一道壁垒，新玩家从零开始至少 3 个月 |
...
2. **Weekly Triage**（每周一）："周一 8:45–8:55 这 10 分钟，让我一眼看清本周谁最急、为什么、下一步查什么。"
...
| F-3 | 人工确认 deadline 规则库（Federal + CA/NY/TX/FL/WA，约 30 条规则） | 每条规则有 source_url + verified_by + verified_at |
...
| F-15 | 最小 audit log（所有 Apply / 状态变更 / Migration Import） | 可在客户详情页和 Settings → Imports 看到时间线 |
...
| F-18 | Penalty Dollar Forecaster（基于 IRS 5%/0.5% 规则估算漏报 $ 风险） | 挪到 Phase 2；不与 Migration 抢 14 天时间 |
...
（不做"AI 自动决定优先级"——v0.3 边界不允许；但分组规则由人类工程师实现，AI 只写文案）
...
> *Paste from Excel, Numbers, Google Sheets, or any table. We'll figure out the columns.*
...
```json
{
  ...
  "per_row": [
    {
      "row_index": 0,
      "values": { "client_name": "Acme LLC", "state": "CA", "entity_type": "LLC", "email": "john@acme.com" },
      "confidence": 0.98,
      "notes": "all fields directly mapped"
    },
    {
      "row_index": 2,
      "values": { "client_name": "Miller Family Partnership", "state": "TX", "entity_type": "Partnership" },
      "confidence": 0.75,
...
- 位置：导入后 7 天内，Settings → Imports 页每条记录右侧有 Undo 按钮
...
销售讲：
> *"Show me your client list in whatever format you have."*
...
App (after login)
 ...
 ├─ Rules （只读 + source/verified/last_checked）
 ├─ Radar （所有历史公告 + 状态）
 └─ Settings
     ├─ Profile
     ├─ Notifications
...
| D3 | Rule DB seed（Federal + CA + NY + TX + FL + WA，人工录入 ~30 条）+ Deadline 自动生成 | 创建客户后立刻有 deadlines |
| D4 | Weekly Triage Console 首页（Critical/High/Upcoming 三段 + 静态排序） | 首页可看 |
| D5 | 状态 + Readiness + Internal cutoff + 筛选 | 核心 CRUD 闭环完成 |
| D6 | AI 层骨架（LiteLLM + prompts + llm_logs）+ Weekly Brief + Deadline Tip + Client Risk Summary | AI 能出东西 |
...
| D11 | 邮件提醒（Resend）+ 应用内提醒 + Cmd-K | 通知闭环 |
...
**指标逻辑**：Migration 3 指标验证"能进来"；Triage + Brief/Tips 指标验证"每周有用"；Radar Apply 验证"AI 智能被接受"；$49 意愿验证"愿付费"。四层叠加才构成 Proceed 信号。
...
| Radar 只抓 IRS + 5 州，用户客户恰好在别州 | 失望 | UI 明确显示"Currently watching: IRS + CA/NY/TX/FL/WA"，让用户知道边界 |
...
| 手动客户录入 | ✓ F-2 |
| Federal + CA/NY/TX/FL/WA 人工规则 | ✓ F-3 |
| 首页优先级分诊台 | ✓ F-5 |
...
- **工程代价**：D7–D8 两天，通过 D9 压缩 Radar 的 1 天和移除 Penalty Forecaster 抵扣。
...
**请在 D1 开工前与合规/产品对齐一次这两处升级。** 如果任一被推翻，替代路径：Migration 降级为仅 CSV 文件上传 + 静态字段映射（无 AI）；Radar 降级为人工录入 alert。
...
```
Year 1 (MVP)   : 1 州人工规则 + IRS + 5 州 Radar + AI Triage
