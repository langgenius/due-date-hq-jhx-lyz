            ...
            <TableRow key={invitation.id} className="h-14">
              ...
              <TableCell className="py-2 font-mono text-xs leading-4">
                <span className="block text-text-secondary">
                  <Trans>Sent {formatShortDate(invitation.createdAt)}</Trans>
                </span>
                <span
                  className={cn(
                    'block',
                ...
                >
                  {invitation.status === 'expired' ? (
                    <Trans>Expired {formatShortDate(invitation.expiresAt)}</Trans>
                  ) : (
                    <Trans>Expires {formatShortDate(invitation.expiresAt)}</Trans>
                  )}
                </span>
              </TableCell>
              <TableCell className="py-2">
...
function isManagedRole(value: unknown): value is MemberManagedRole {
  return value === 'manager' || value === 'preparer' || value === 'coordinator'
}

function roleLabel(role: MemberPublic['role'] | MemberManagedRole): string {
...
function invitationDescription(invitation: MemberInvitationPublic): string {
  if (invitation.status === 'expired') return 'Link expired · ask Owner to resend'
  return 'Magic-link delivered · awaiting accept'
}

function inviterName(members: MemberPublic[], inviterId: string): string {
