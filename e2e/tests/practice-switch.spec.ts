...
// Feature: Practice creation and switching
// PRD: P1 tenant foundation
// AC: E2E-PRACTICE-CREATE-SWITCH
...
test('AC: E2E-PRACTICE-CREATE-GATE blocks extra self-serve practice creation', async ({
  authenticatedPage,
}) => {
...
test('AC: E2E-PRACTICE-CREATE-SWITCH creates a separate practice on the Firm plan', async ({
  ...
  const dialog = authenticatedPage.getByRole('dialog', { name: 'Add practice' })
  await expect(dialog).toBeVisible()
  await dialog.getByLabel('Practice name').fill(firmName)
  await dialog.getByLabel('Timezone').click()
  await authenticatedPage.getByRole('option', { name: /Pacific.*America\/Los_Angeles/ }).click()
  await dialog.getByRole('button', { name: 'Create practice' }).click()

  await expect(dialog).toBeHidden()
  await expect(
    authenticatedPage.getByRole('button', {
      name: new RegExp(`Switch practice.*${escapeRegExp(firmName)}`),
    }),
  ).toBeVisible()
