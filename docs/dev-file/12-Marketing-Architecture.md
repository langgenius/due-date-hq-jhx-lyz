...
- `robots.txt` 在 `public/robots.txt` 显式声明，指向 `https://duedatehq.com/sitemap-index.xml`，并显式允许 Googlebot、OAI-SearchBot、GPTBot、ClaudeBot、Claude-SearchBot 和 PerplexityBot 访问公开站。
- `public/llms.txt` 作为 AI-readable content map，列出核心公开页、州覆盖页、引用边界、官方来源策略和非税务建议声明。它只是辅助入口，不替代 sitemap / robots / canonical。
