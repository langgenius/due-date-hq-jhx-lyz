...
- 步骤：
  ...
  3. `workspace-build`：先 build `apps/app`，再跑 `apps/server` Wrangler dry-run，最后 build
     独立的 `apps/marketing`
  4. D1 迁移 `pnpm db:migrate:remote`（从 `apps/server/wrangler.toml` 的 `DB` binding 解析
     `due-date-hq-staging`，并显式带 `--remote`）
  5. `wrangler deploy --env=""` 发布 SaaS Worker + SPA assets
  6. Deploy marketing static Worker，CTA 指向 `https://app.due.langgenius.app`

**Staging 资源前置条件：**

- `apps/server/wrangler.toml` 中声明的 D1 / KV / R2 / Vectorize / Queue 资源必须已在
  Cloudflare account 中存在；CI 只应用 D1 migration 和部署 Worker，不自动创建 binding
  资源。
- Dashboard AI Brief 需要两个新增 Queue 资源：
  - `due-date-hq-dashboard-staging`
  - `due-date-hq-dashboard-dlq-staging`
- 这两个 Queue 不是 secret，不需要新增 GitHub Actions secret。首次部署前可用：

```bash
pnpm --dir apps/server exec wrangler queues create due-date-hq-dashboard-staging
pnpm --dir apps/server exec wrangler queues create due-date-hq-dashboard-dlq-staging
```
...
- GitHub environment `due-date-hq-staging` secrets：`CLOUDFLARE_API_TOKEN` · `AUTH_SECRET` ·
  ...
  `STRIPE_PRICE_FIRM_MONTHLY`。可选：`RESEND_API_KEY`、`STRIPE_PRICE_FIRM_YEARLY`、
  `STRIPE_PRICE_PRO_MONTHLY`、`STRIPE_PRICE_PRO_YEARLY`；AI Gateway OpenRouter 路径需要
...
- Stripe staging 可以使用 Test mode：`STRIPE_SECRET_KEY` 使用 `sk_test_*`，
  `STRIPE_PRICE_FIRM_MONTHLY` 使用同一 test mode 下创建的 recurring price id，
...
Dashboard / Clients / Workboard / Team Workload / Rules Console / Members / Audit Log /
Migration Step 1 / firm switch / billing checkout 与 webhook-backed success 流。真实 Google OAuth
