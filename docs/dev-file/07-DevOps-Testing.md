...
- 步骤：
  ...
  3. `workspace-build`：先 build `apps/app`，再跑 `apps/server` Wrangler dry-run，最后 build
     独立的 `apps/marketing`
  4. Queue preflight `pnpm cf:ensure-queues`：读取 `apps/server/wrangler.toml` 中的
     `queues.producers` / `queues.consumers` / `dead_letter_queue`，用 Wrangler 只创建缺失
     的 Queue 资源
  5. D1 迁移 `pnpm db:migrate:remote`（从 `apps/server/wrangler.toml` 的 `DB` binding 解析
     `due-date-hq-staging`，并显式带 `--remote`）
  6. `wrangler deploy --env=""` 发布 SaaS Worker + SPA assets
  7. Deploy marketing static Worker，CTA 指向 `https://app.due.langgenius.app`
