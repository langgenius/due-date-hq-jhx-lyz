...
| **production** | Cloudflare Pages / static Worker (`duedatehq.com`) | top-level `duedatehq` (`app.duedatehq.com`) | `duedatehq`           | top-level bindings  | `pnpm deploy`                    |
...
- 步骤：
  ...
  3. `workspace-build`：显式 `apps/marketing`、`apps/app` build 后再跑 `apps/server` Wrangler dry-run
  4. Deploy marketing 到 `duedatehq.com`（Cloudflare Pages direct upload 或 static Worker）
  5. D1 迁移 `wrangler d1 migrations apply duedatehq --remote`（只允许 expand/forward-compatible migration）
  6. `wrangler deploy --env=""` 发布 SaaS Worker
