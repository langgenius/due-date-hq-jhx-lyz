...
- GitHub environment `due-date-hq-staging` secrets：`CLOUDFLARE_API_TOKEN` · `AUTH_SECRET` ·
  ...
  `STRIPE_PRICE_PRO_MONTHLY`。可选：`RESEND_API_KEY`、`RESEND_WEBHOOK_SECRET`、
  `STRIPE_PRICE_PRO_YEARLY`、
  `STRIPE_PRICE_SOLO_MONTHLY`、`STRIPE_PRICE_SOLO_YEARLY`、`STRIPE_PRICE_TEAM_MONTHLY`、
  `STRIPE_PRICE_TEAM_YEARLY`、`STRIPE_PRICE_FIRM_MONTHLY`、`STRIPE_PRICE_FIRM_YEARLY`；AI
  Gateway OpenRouter 路径需要 `AI_GATEWAY_PROVIDER_API_KEY`。`AI_GATEWAY_API_KEY` 仅在启用
...
- Stripe staging 可以使用 Test mode：`STRIPE_SECRET_KEY` 使用 `sk_test_*`，
  `STRIPE_PRICE_PRO_MONTHLY` 使用同一 test mode 下创建的 recurring price id。Solo / Team
  checkout 只有在对应 `STRIPE_PRICE_*` price id 存在时才启用；webhook endpoint 指向
