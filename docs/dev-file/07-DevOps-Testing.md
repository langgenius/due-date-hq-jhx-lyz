...
- `.github/workflows/e2e.yml`：独立 E2E CI，不并入主 `ci.yml`。

当前已落地 smoke 只覆盖真实存在的无登录场景：Hono health、登录入口、受保护路由未登录跳转、
marketing `lng` handoff、入口语言切换、SPA 404。Google OAuth、Dashboard、Workboard、
Migration Copilot happy path 等登录态业务流等 Better Auth/D1 seed fixture 落地后再补。
