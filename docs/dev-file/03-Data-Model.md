...
| 字段                                                                                               | 备注                                                                                                                                                           |
| -------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `id` / `firm_id` / `client_id` / `rule_id` / `rule_version`                                        | `rule_id` / `rule_version` are nullable for legacy/manual rows; annual rollover writes verified rule metadata                                                  |
| `tax_year` / `rule_period` / `generation_source`                                                   | `generation_source ∈ (migration, manual, annual_rollover, pulse)`；`firm_id + client_id + rule_id + tax_year + rule_period` supports idempotent generated rows |
