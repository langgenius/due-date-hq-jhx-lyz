...
| 资产             | 当前数量 | 说明                                                                  |
| ---------------- | -------- | --------------------------------------------------------------------- |
| `RuleSource`     | 22       | Federal + CA / NY / TX / FL / WA 官方来源；含采集方式、频率、通知通道 |
| `ObligationRule` | 21       | 20 条 verified rule + 1 条 Federal disaster relief candidate          |
| 覆盖州           | 6        | `FED`、`CA`、`NY`、`TX`、`FL`、`WA`                                   |
| 消费 API         | 3 个     | `rules.listSources`、`rules.listRules`、`rules.coverage`              |

当前实现仍然保持纯 domain asset，不直接写 D1。原因是现有
`obligation_instance` schema 仍处在 Demo Sprint 阶段，PRD 中的
`rule_id -> obligation_rule.id` 回填属于下一步 obligations 集成任务。
