...
用户侧只在 Dashboard / Obligations / Deadline Detail 中消费 verified 结果：source badge、human verified、quality tier、AI Tip、next check。
...
| 版本 | 日期       | 作者  | 摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ---- | ---------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v1.2 | 2026-05-04 | Codex | Obligation Preview 增加手动 Annual Rollover 面板：先 preview 再 generate，不做 cron；closed source obligations 作为 seeds，目标年度必须有 verified rule；duplicate / missing verified rule / missing due date 都在表格中显式展示，创建后写 evidence、audit，并刷新 Dashboard brief。                                                                                                                                                                                                                                                                          |
...
| v0.5 | 2026-04-28 | Codex | 布局判定从 URL 前缀切换到内容形态：Rules Console 是 ops data surface，全宽展开（去掉 `mx-auto max-w-[928px]`），与 tab nav / Obligations 共锚 `left=24`。Coverage tab 重做为 KPI 条 + 左 7/12 jurisdiction summary + 右 5/12 jurisdiction × entity 矩阵；KPI 数字走 `font-mono text-2xl tabular-nums`，candidate>0 时数字 tone 走 `text-status-review`。`RulesPageHeader` 段落 max-w 从 720 → 1080，避免在 1512 视口下挤成 3 行。`DESIGN.md` §5.2 同步增订按内容形态判定宽度的规则。详见 `docs/dev-log/2026-04-28-rules-console-fullwidth-coverage.md`。      |
