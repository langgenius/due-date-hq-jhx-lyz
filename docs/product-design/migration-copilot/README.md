...
本册的一句话定位：**把 Migration Copilot 的产品形态、UX、AI Prompt、Default Matrix、Fixture、设计系统增量在 Demo Sprint 口径下一次性封死。**

本册是 PRD §6A（Migration Copilot）与 §6A.11（Onboarding AI Agent）的**落地翻译 + 冲突裁定 + Demo Sprint 子集化**，**不是新 PRD**。PRD 依然是产品语义的最终权威，本册负责把"4 周 Phase 0 MVP 范围"收窄到"7 天 Demo Sprint 可跑通的最小闭环"，并把跨文档、跨 AC、跨工程模块之间的歧义以"裁定表 + 子文档索引"方式一次性固定。
...
本册共 10 份子文档（01 ~ 10），路径全部相对于 `docs/product-design/migration-copilot/`。本 README 是总入口；任何实现 / 文案 / 数据定义分歧先读 `./10-conflict-resolutions.md`。
...
> 说明：02 ~ 09 与 ADR 0011 由其他 subagent 产出；本文件中的子文档链接是**契约位**，允许尚未写入仓库，先行就位。任何子文档新增 / 拆分 / 合并必须先改本 README §2 的清单再改子文档（见 §6 维护约定）。
...
> Phase 0 扩展位：当 Demo Sprint 结束、进入 4 周 Phase 0 MVP 时，本 README 需要增补：辖区扩 seed 清单（MA / TX / FL / WA）、Manager 权限开闸表、Overlay Engine 迁移路径、Onboarding AI Agent 真实化 rollout 计划。上述 hook 由 `../../dev-file/09-Demo-Sprint-Module-Playbook.md` §14 Phase 1 接续保留入口，**本轮不展开**。
