...
| 5 个 Preset Profile 含 File In Time（Part1B §6A.4）                                                                                                                                                                                                                   | TX / FL / WA matrix 扩展（本 Sprint 仅 Federal + CA + NY；当前 Rules MVP 已覆盖 TX / FL / WA）                |
...
| Coverage transparency：Step 3 / Step 4 显示 `verified / demo_seed / skeleton / unsupported` 状态（见 [`./11-agentic-enhancements.md#3-增强点-b--6-辖区信任路线coverage-transparency`](./11-agentic-enhancements.md#3-增强点-b--6-辖区信任路线coverage-transparency)） | Default Matrix 扩展到 CA / NY / TX / FL / WA，并与当前 Rules MVP coverage 对齐                                |
...
> Phase 0 扩展位：TX / FL / WA matrix 扩展（扩展 `./05-default-matrix.md`）；Manager / Preparer 入口可见性开关；Pulse apply 事件到达时在向导 Step 4 Genesis 后显示 `1 rule updated recently` banner 与 Migration 产生的 obligations 联动。当前 Rules MVP coverage 已是 Federal + CA/NY/TX/FL/WA，MA/IL 不属于当前 MVP。
...
| Coverage transparency     | CA / NY 标 `demo_seed`，TX / FL / WA 标 `skeleton` 或 `state_review_needed`；Rules Console 按当前 verified rules 展示 | 5 MVP states 逐格 ops verified         | 把 pending coverage 说成 verified         |
...
- Step 1 增加 `Paste / Upload`、`Provider export`、`Reuse provider import` 三入口。
- `Provider export` 属于 activation migration import，用于供应商导出文件、报表或 integration
  handoff payload，不等同于 Team+ 的 guided integration，也不承诺第三方平台一键全量同步。
