...
```text
Paste / CSV
  -> AI or preset mapping
  -> normalize entity/state/tax types
  -> Default Matrix fills missing tax_types
  -> verified rules generate obligations
  -> Workboard / Dashboard show weekly triage
```
...
- 每条 deadline 的依据来自 verified rules / official sources。
...
4. 使用 verified rules preview 生成 obligations；AI 和 matrix 都不能直接生成 due date。
5. 写入 clients、obligations、verified rule evidence、audit events，并更新 batch 为 `applied`。
6. 返回 created counts 与 24h revert 截止时间。
...
- **Rules 优先**：日期来自 verified rules；Default Matrix 只推断 tax types。
- **坏行不阻塞好行**：跳过行保留 error reason，成功行继续入库。
- **证据先行**：每条 generated obligation 至少有一条 `verified_rule` evidence。
- **审计可回看**：写入 `migration.imported`、`client.batch_created`、`obligation.batch_created`。
...
用户粘贴 20-30 行客户表
  ...
  -> Import & Generate 后 Workboard 出现真实 obligation rows
...
- 每条 obligation 至少可追溯到 verified rule evidence。
