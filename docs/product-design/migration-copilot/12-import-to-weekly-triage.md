...
Paste / CSV
  ...
  -> Workboard / Dashboard show real weekly triage
```
...
6. 返回 created counts 与 24h revert 截止时间；前端可在 import success toast 上提供
   最小 `Undo import` 入口，并 invalidate / prefetch Dashboard + Workboard。
...
- `dashboard.load` 对同一 firm 返回真实 summary/top rows；Undo import 后 Dashboard / Workboard 都不再显示该批 obligation。
