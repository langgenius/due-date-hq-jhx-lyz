...
| Demo 只有 Federal + CA + NY            | ICP 常见 TX / FL / WA / MA 未被正面回应 | 做 6 辖区信任路线和 coverage transparency                   |
...
| `previewCoverage`     | 返回 Federal/CA/NY/TX/FL/WA/MA coverage 状态  | 否                  | 不得声称未验证州已 verified         |
...
| Phase 1     | 完整 7-state Agent、Setup History、A/B funnel、跨标签 draft 恢复                                                                | Agent 税务判断、无证据生成 deadline         |
...
## 3. 增强点 B · 6 辖区信任路线（Coverage Transparency）

### 3.1 为什么要做

PRD 主 ICP 至少有 CA / NY / TX / FL / WA 多州客户。Demo Sprint 只做 Federal + CA + NY 是工程可控选择，但产品体验不能让用户误以为 DueDateHQ “只懂两个州”。
...
Demo Sprint 的 CA / NY 可以保持 `demo_seed`；Phase 0 正式试点前必须升级为 `verified`。TX / FL / WA / MA 在 Demo Sprint 至少进入 `skeleton`，让 Agent 能诚实回答“我们知道你有这些州，但需要 review”。
...
如果存在 approved Pulse 匹配新导入客户，Dashboard Banner 展示受影响客户；如果只是 pending_review Pulse，不进入 Apply 集合，只进 review strip。
...
- Federal + CA / NY / TX / FL / WA / MA matrix cell 全部进入 `verified` 或 `skeleton`，不得有无状态 cell。
