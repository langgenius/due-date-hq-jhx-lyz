...
- 数字字段全部走 `Intl.NumberFormat(INTL_LOCALE[locale], ...)` 格式化（对齐 `adr/0009-lingui-for-i18n.md` §SPA 的 `INTL_LOCALE` 映射，Worker 的 `src/i18n/messages.ts` 薄字典在插值前调用同一格式化函数）
...
- SPA 的 `.po` catalog **不复用**；两套字符串同源不同仓（对齐 ADR 0009 §代价 第 3 条）
