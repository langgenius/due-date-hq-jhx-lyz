...
| 05 Frontend Architecture | SPA workbench；React Router；TanStack Query/Table；Zustand 不超过 3 个 store；PWA 约束 | 前端模块以业务能力隔离，server state 不进 Zustand |
...
| Pay-intent + PWA | `$49/mo` 点击事件 + installable PWA | §7.8.1 + §15.3.6 |
...
- Pay-intent is clearly not Stripe checkout。
...
|---|---|---|---|
| Client Domain Contract | Client + Workboard | Migration、Dashboard、Pulse、Demo Seed | 新字段优先 optional；client identity 不由 consumer 自造 |
...
flowchart TD
  ...
  PWA["PWA + Push"]
  Demo["Demo Data + Pay-intent + Polish"]
...
module/pulse · module/pwa · module/demo · module/devops
...
涉及 Worker binding、Queue、PWA 或 Push 的 PR 追加 Workers integration smoke。

默认质量链路：

- `oxfmt` for formatting。
- `oxlint` for lint and import-boundary rules。
- `tsgo` for default typecheck。
- Vitest for unit/integration tests。
- gitleaks for secret scanning。
- `tsc --noEmit` only as `check-types:stable` fallback。
- No default `tsgolint` or typed ESLint gate。
...
| Push 到不了设备 | in-app notification fallback | 阻塞 Pulse 主闭环 |
