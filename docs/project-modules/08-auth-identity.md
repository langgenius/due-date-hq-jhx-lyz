...
`packages/auth` 封装 DueDateHQ 的 Better Auth 配置、组织插件、角色权限矩阵和可选 Stripe plugin。`apps/server` 在 Worker runtime 中创建具体 auth instance，并负责注入 D1 database、邮件发送、locale、organization hooks 和 billing hooks。
...
- Google OAuth；可选 Microsoft Entra ID OAuth（`MICROSOFT_CLIENT_ID/SECRET` 成对配置后显示）。
- Better Auth session。
...
邀请邮件落到 `/accept-invite?id=...`。用户未登录时先走 Google/Microsoft OAuth，登录后调用 Better Auth `accept-invitation`，接受后回到受保护工作台。
...
- `enableTwoFactor` / `verifyTwoFactor` / `disableTwoFactor`：驱动 `/account/security`。
...
flowchart TB
  ...
  Plugins["organization + twoFactor + microsoft + stripe plugins"]
