...
`packages/auth` 封装 DueDateHQ 的 Better Auth 配置、Email OTP、Google OAuth/One Tap、组织插件、角色权限矩阵和可选 Stripe plugin。`apps/server` 在 Worker runtime 中创建具体 auth instance，并负责注入 D1 database、邮件发送、locale、organization hooks 和 billing hooks。
...
- Email OTP passwordless 登录/注册：`/email-otp/send-verification-otp` 只允许 `type='sign-in'`，`/sign-in/email-otp` 验证 6 位验证码；新邮箱允许自助注册并进入现有 onboarding。
- Google OAuth + Google One Tap；可选 Microsoft Entra ID OAuth（`MICROSOFT_CLIENT_ID/SECRET` 成对配置后显示）。
- `/api/auth-capabilities` 暴露公开 provider 能力与 Google Client ID 给 SPA 初始化 One Tap；`GOOGLE_CLIENT_SECRET` 仍只存在于 Worker secret/runtime env。
- Email OTP 复用 Better Auth 既有 `verification` / `user` / `session` / `rate_limit` 表，不引入新的 auth 表或迁移。
- Better Auth session。
...
邀请邮件落到 `/accept-invite?id=...`。用户未登录时可走 Email OTP 或 Google/Microsoft OAuth（登录页可先显示 Google One Tap），登录后调用 Better Auth `accept-invitation`，接受后回到受保护工作台。
...
flowchart TB
  ...
  Plugins["organization + twoFactor + emailOTP + oneTap + microsoft + stripe plugins"]
