...
- Team 和 Enterprise 计划会额外显示 Priority Queue，用于把低置信度、needs review、高影响、
  source attention 或 preparer requested 的 Pulse 提醒排到经理审核优先队列。
...
5. Team/Enterprise 可切换 `All Pulse` / `Priority Queue`；Pro 看到 Team-only locked state。
6. 点击 alert 的 `Review` 打开详情侧栏。
7. 在详情中检查：
...
8. 对需要人工确认的客户进行勾选确认。
9. Team/Enterprise Owner/Manager 可在 manager review panel 中：
   - `Confirm all review-needed` 批量确认同一个 Pulse alert 内的 needs review 客户。
   - 对单个客户 confirm 或 exclude。
   - `Save manager review` 持久化已审核集合，不立即改 deadline。
   - `Apply reviewed set` 只把已保存集合应用到现有 Pulse apply/evidence/audit 流程。
10. Owner/Manager 仍可使用基础 Pulse 操作：`Apply to # clients`、`Dismiss`、`Snooze 24h`、
    `Undo (24h)`、`Reactivate / Re-apply`、`Copy client email draft`。
11. Apply 成功后会写 audit、链接 evidence，并排队 digest。
...
6. `Pulse Changes` tab：
   ...
   - Team/Enterprise 可打开 `Priority Queue`，按风险分数处理需要经理审核的 Pulse，并在单个
     alert 范围内保存 review set 后再 apply。
...
Practice profile 管理当前 practice 的名称和 timezone，并提供删除当前 practice 的操作。该配置只作用于当前工作区，历史审计记录会保留。

**适用用户**

- 具备当前 practice 访问权的用户可查看当前 practice 摘要。
- Owner 可修改 practice name、timezone，并可删除当前 practice。
...
2. 筛选 active changes。
3. Team/Enterprise 可切换到 `Priority Queue`，优先处理风险分数高的 Pulse。
4. 点击 `Review` 打开 Pulse detail。
5. 检查 confidence、source URL、structured fields 和 affected clients。
6. 对 needs review 客户确认选择。
7. Team/Enterprise Owner/Manager 可保存 manager review，并用 `Apply reviewed set` 应用已审核集合。
8. Owner/Manager 也可点击 apply、dismiss 或 snooze。
9. 如应用错误，在 24 小时窗口内执行 undo。
10. 在 Audit log 中查看对应 Pulse 决策。
