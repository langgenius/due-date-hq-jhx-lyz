...
3. 查看 `Penalty Radar` 的本周风险金额、ready、needs input、unsupported 数量。
4. 在 `Risk pulse` 中查看最紧急的 obligation。
...
Rules Console 展示 DueDateHQ 当前使用的规则覆盖、权威来源、规则库和 generation preview。它主要用于解释规则从哪里来、哪些规则 verified、哪些需要人工审核，以及给定客户事实会生成哪些义务。
...
5. `Generation Preview` tab：
   - 输入 client id、entity、state、tax year、tax types。
   - 点击 `Run preview`。
