...
- 让团队在 Dashboard / Obligations / Audit 中分诊截止日、风险、证据和负责人。
...
| 2. 排期                      | 部分覆盖   | Obligations 有 due date 和负责人，但没有客户资料提交窗口、目标交付周、产能排班、season wave                                         |
...
| 5. 补资料请求                | 半成品雏形 | readiness contract/schema/repo/server handler 已出现，但缺迁移、公开 portal 路由、Obligations detail 挂载、前端操作入口和端到端验证 |
...
- Obligations 的 due date、days、status、readiness、assignee。
...
- Admin queue：让 coordinator/admin 主导资料窗口和提醒，而不是让 CPA 在 Obligations 中手工判断。

建议新增 `season_schedule` 或 `return_project_schedule` 模块，让 Obligations 的 due date 视角叠加
内部产能和交付承诺。
...
入口、文件分类、缺件追踪、客户 portal，也没有与 Obligations 的 `readiness` 做成闭环。
...
- `packages/contracts/src/obligations.ts` 中的 `readinessRequests`
...
- `apps/server/src/procedures/obligations/index.ts` 仍没有实现 contract 里的 `getDetail`，root router
  也未挂载 `obligations.getDetail`。
- `apps/server/src/lib/readiness-token.ts` 已有 token helper，但当前没有公开 `/readiness/:token`
  portal route 来展示和提交客户响应。
- `apps/app/src/routes/obligations.tsx` 仍只暴露 readiness 状态筛选和更新，尚未提供“生成
  checklist、发送客户请求、查看响应、撤销请求”的 UI。
- 目前缺少围绕 readiness request、portal submit、邮件发送、audit/evidence 和 Obligations
  状态同步的端到端测试。

因此 readiness 已经是正确方向，但仍是半成品状态。继续开发前应先把 migration、public
portal、Obligations detail、UI 和测试闭环，否则会扩大类型和运行时漂移。
...
- Obligations export PDF 不是 return preparation packet。
...
- 从 binder 推送 Obligations readiness 状态。
