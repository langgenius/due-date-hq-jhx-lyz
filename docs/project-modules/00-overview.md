...
- **Workboard / Dashboard / Workload**：把义务状态、罚金风险、负责人、证据和团队负载变成日常操作台。
- **Pulse**：从 IRS、州税务机构、FEMA 等来源抓取更新，经过 ops review 后生成事务所可处理的 alert，并能应用到客户义务。
...
| 能力域       | 用户价值                                          | 当前实现入口                                                                        |
| ------------ | ------------------------------------------------- | ----------------------------------------------------------------------------------- |
| 身份与事务所 | 支持组织、成员、角色、active firm 切换            | `apps/server/src/auth.ts`, `packages/auth`, `apps/app/src/routes/firm*`, `members*` |
...
| 区域             | 状态             | 说明                                                                                                      |
| ---------------- | ---------------- | --------------------------------------------------------------------------------------------------------- |
| 前端主工作台     | 已实现           | Dashboard、Workboard、Migration、Pulse、Audit、Rules、Members、Billing 等路由存在                         |
...
| Pulse ingest     | 部分实现         | adapter/fetcher 框架完整，source 覆盖多州和 IRS；ops review 流程已存在                                    |
...
- 给 `packages/core/priority` 补齐 Smart Priority scoring，并在 Workboard/Dashboard 中形成一致解释。
