...
SPA 采用 **Lingui v6**；Worker 采用**类型化字典 shim**（不引入任何 Lingui
运行时）：
...
8. **富文本占位符命名。** `apps/web/lingui.config.ts` 启用 Lingui v6 的
...
- `@vitejs/plugin-react` v6 已改用 SWC，无法承载 Lingui macro，因此需要
  再挂一个 `@rolldown/plugin-babel` 做 macro 转换。它只覆盖 `apps/web/src`
