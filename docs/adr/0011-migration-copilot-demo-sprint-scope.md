...
| --- | ---------------------------- | ------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| 1   | Revert 24h 全量权限          | Part1A §3.6.3 RBAC + Part1B §6A.7 Revert 双档                | **Owner-only**（Demo Sprint + Phase 0 MVP 一致）；Revert 单客户 7d = Owner + Manager                                                            | [10 §1](../product-design/migration-copilot/10-conflict-resolutions.md#1-revert-24h-全量撤销权限)         |
...
- Migration 专属 LLM 配额默认 20 次/day 是本册估算值；`../dev-file/04-AI-Architecture.md` §8 只写"每 batch 固定开销"，真实上线前需调参（**FU-2**）
