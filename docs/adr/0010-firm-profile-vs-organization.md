...
也就是说 PRD 第一性把这些字段视为 **first-class**（结构化、可索引、可加 CHECK
约束、可加 FK），不是 metadata blob。
...
3. P0-23 Pay-intent 接 Stripe 时，`billingCustomerId` 要做唯一索引、要支持双
...
firmId)` 的调用点都得选哪个 id。PK 复用换来的"一个值贯穿所有层"是第一性
  ...
  场景，那时单独写一个 ADR 反向重构。
- **(c) 在 `organization` 表里 ALTER 添加业务字段** —— **拒绝**。better-auth
  的 organization 表由它的 migrations 管理（dev-file/03 §1.4 已写明），加列
  会在 better-auth 升级时被覆盖；分表无此风险。

## 后果（Consequences）

### 好处

- 业务租户字段 first-class（可索引、可 CHECK、可 FK），P1 逻辑可以直接用
- `firmId` 单一语义不变，下游零改动
- Pay-intent / billing 接入时 `billingCustomerId` 等列直接 ALTER 即可，不需要
  迁移历史数据
...
| ID   | 触发时机                                      | 内容                                                                                                                        |
| ---- | --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| FU-1 | P1 接邀请流（PRD §3.6.4）                     | `membershipLimit` 改成函数式：`(user, org) => planSeatLimit(org.plan)`；删 `beforeAddMember` 的硬约束                       |
| FU-2 | 第一个真实 repo 落地（clients / obligations） | 把 `scoped(db, firmId)` 签名扩成 `scoped(db, tenant: TenantContext)`，让 repo 直接读 `tenant.plan` 而不是从 `c.var` 取      |
...
| FU-4 | P0-23 Pay-intent                              | 给 firm_profile 加 `billing_customer_id` / `billing_subscription_id` 列，写 Stripe webhook → firm_profile 同步路径          |
