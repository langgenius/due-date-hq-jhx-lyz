...
1. **TopNav 布局**：`Brand + NavLinks` 组与 `Status + Sign in + Open app` 组用 `justify-between` 拉到两端；`Brand → NavLinks` 之间用 `gap-8`（32px，对齐 Figma `NavLeft` 段 `Brand` 238w 后的 32px 间距）。**严禁**从 Figma metadata 的 `NavLinks.x = 270`（绝对坐标）误读为 gap 值，那会让 nav 链接被甩到右侧贴 Status pill。
...
| FU-5 | 第一个用户从 marketing CTA 点进 app 时 | 验证跨域跳转 `duedatehq.com → app.duedatehq.com/login` 不丢失 PostHog distinct_id（[`../dev-file/12-Marketing-Architecture.md`](../dev-file/12-Marketing-Architecture.md) §6 cross-domain identity merging 已设计但未落地）                                                      | Analytics owner                               |
