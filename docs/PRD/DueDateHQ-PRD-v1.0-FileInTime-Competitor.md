...
| **F-18** | **★ Penalty Dollar Forecaster（详见 §5.9）**：基于 IRS 5%/月 + 0.5%/月规则（MVP 硬编码联邦 + 5 州主要表单）估算每条 Critical / High deadline 若漏报的最大罚款金额区间                | 每条 Critical/High 卡片右下角显示 `~$1,200–$3,000 at risk` 胶囊；点击展开计算依据与官方规则链接；作为 F-5b AI 排序的核心输入 |
...
| D4     | Weekly Triage Console 首页（Critical/High/Upcoming 三段 + 静态排序）+ **Penalty Forecaster 硬规则表（F-18，联邦 + 5 州主要表单的 `penalty_formula` 入 rule 表）+ TriageCard 胶囊展示** | 首页可看 + 每条 Critical/High 有 `at risk $` |
...
- **工程代价**：
  - F-18 `penalty_formula` 字段嵌入 rule 表 D4 完成（seed 时一次性填入联邦 + 5 州主要表单），前端胶囊 D4 完成；
