# 2026-04-26 · Day 3 收口（JHX Migration 坏行可见 + LYZ Workboard 状态流接通）
...
2. `obligations.updateStatus` 与 `workboard.list` 的契约 + handler。
3. Workboard 路由的真实数据接入（之前是静态 mock，状态枚举与 DB 不一致）。
...
- `scoped.ts` 把 `workboard` 挂到 `ScopedRepo`；`types.ts` 同步类型。
- `repo/workboard.test.ts` 用 fake-Drizzle chain 覆盖：limit 边界、cursor sentinel、
...
- `procedures/workboard/index.ts`：薄 handler，仅做 Date → ISO 字符串映射，
  把 `WorkboardListResult` 转成契约 shape。
- `procedures/index.ts`：把 `workboard: {}` 替换为真实 handlers，注册
...
- `db/db.test.ts` 加 `obligations.findById` + `workboard.list` smoke。
...
### 6. 前端 workboard（`apps/app/src/routes/workboard.tsx`）
...
- 删除静态数组与 `'blocked' | 'in review' | …` 假枚举，全部改用 DB 权威的
  ...
  `done` / `not_applicable`）。
- 顶栏：`useQuery(orpc.workboard.list.queryOptions)` 拉数据；多选状态 chips、
  搜索框（`useDebounced` 300ms）、排序下拉。
- 行内：`Select` 触发 `useMutation(orpc.obligations.updateStatus.mutationOptions)`，
  成功后 `invalidateQueries(orpc.workboard.list.key())` + `toast.success` 把
  `auditId.slice(0, 8)` 显示在 description 里 —— 这就是「Workboard 改一条 status
...
- 前端 workboard 没有 `useQuery` 桩测 —— 项目里没有现成的 oRPC + RTL 测试
...
- Day 3 验收两条 `[ ]` 全部勾上；Day 4 起 LYZ 可以基于真实的 Workboard list
...
- Day 4 LYZ：Dashboard 上叠加 server-side aggregation 时复用 `workboard`
