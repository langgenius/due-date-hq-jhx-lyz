---
title: 'Day 3 收口的技术设计：listErrors RPC、Workboard cursor 分页与 audit 不变量'
date: 2026-04-26
author: 'Codex'
---

# Day 3 收口的技术设计：listErrors RPC、Workboard cursor 分页与 audit 不变量

## 背景

Day 3 收口落地见 [2026-04-26 day3 closeout](./2026-04-26-day3-closeout.md)（changelog 风格的"做了什么"）。
本文是它的姊妹篇，专注**为什么这样设计**——把这次新增的几个跨层契约
（`migration.listErrors` / `obligations.updateStatus` / `workboard.list`）
...
flowchart LR
  ...
  subgraph workboardLane [LYZ · Workboard]
    page[workboard.tsx]
    page -->|"orpc.workboard.list"| listH[workboard.list handler]
    listH --> wbRepo[scoped.workboard.list]
...
### 5. Workboard cursor 分页：keyset on `(currentDueDate, id)`

**约束**：D1 单条 SQL 100 个 bound 参数，offset 大了之后 query plan
退化（即使有索引，offset N 也要扫 N 行）。Workboard 用户场景有几十到
几百行义务很常见。

**最终方案**：[`packages/db/src/repo/workboard.ts`](../../packages/db/src/repo/workboard.ts)
...
### 6. Workboard 状态枚举权威下移到 DB schema

之前 `workboard.tsx` 自己造了 `'blocked' | 'in review' | 'draft' | 'waiting' | 'filed'`，
...
### 7. WorkboardRow 在 Public schema 上 extend，不另造 DTO
...
`extend` 而不是新造 schema，让 Workboard 行的字段是
"obligation public 字段 + clientName"的超集。前端类型推导直接得到所有
字段；如果未来 `ObligationInstancePublic` 加了 `assigneeName`，Workboard
零代码同步。
...
- **Repo 隔离**：Workboard 的 cursor / sentinel / limit 边界用 fake-Drizzle
...
1. **`migration_error` 加显式 stage 列**：把 `classifyErrorStage` 启发式
   ...
   migration 的好时机。
2. **Workboard 多选状态写型操作**：现在只有单条 `updateStatus`，
...
6. **Workboard 空态 → migration wizard 跳转**：现在通过
