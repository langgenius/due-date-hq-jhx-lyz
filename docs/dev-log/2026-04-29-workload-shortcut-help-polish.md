...
Sidebar IA 已经把 `Team workload` 提升为 `Operations` 下的付费运营面，但全局 `G then ...`
...
`Team workload` 属于日常运营工作流，和 Dashboard / Workboard / Alerts 同层；Firm profile、
Rules、Members、Billing、Audit log 更偏组织管理或审计后台，继续通过 Cmd-K 进入，避免全局
导航序列膨胀。
