---
title: 'Workboard P0-16 status and readiness'
date: 2026-05-02
area: workboard
---
...
P0-16 required the Workboard status flow to cover Filed, Paid, Extended, Waiting, Needs review,
...
- Updated Workboard and Dashboard status labels so `done` renders as Filed, `review` renders as
  Needs review, and `pending` renders as Not started.
- Made Workboard readiness editable inline and in bulk, added Paid as a status target, and added
...
- `pnpm --filter @duedatehq/db test -- src/repo/workboard.test.ts src/db.test.ts src/repo/dashboard.test.ts src/repo/workload.test.ts`
...
- `pnpm check`
- `pnpm check:deps`
- `E2E_REUSE_EXISTING_SERVER=1 pnpm test:e2e e2e/tests/workboard.spec.ts e2e/tests/audit-log.spec.ts`
