...
3. `verifiedAt: '2026-04-27'` 全部一样、`verifiedBy: 'ops.rules.manual'`
   是 placeholder — 没有真实的逐条 ops sign-off 流程。
...
- `verifiedAt` 没改 — 仍是 `'2026-04-27'`。这是 seed pack 的批量 review
  日期（今天）；保留作为约定。
- `verifiedBy` 没改 — 仍是 `'ops.rules.manual'`。重命名成 `'seed.handcurated'`
...
- `verifiedBy: 'ops.rules.manual'` placeholder 应该重命名 + 配合
  contract 加 `lastVerifiedRunId` 之类的字段，让 seed-pack 和真正的 ops
...
- 自动 ingestion / snapshot diff / candidate promotion 流程在
  ...
  缺失）。Phase 2 候选。
- WA 4 条 manual_review source 当前只能 ops 手工巡检。若要纳入自动检测，
