...
- Made Triage queue rows open the exact Workboard obligation detail.
- Linked client names directly to the Clients fact profile for customer-level review.
- Added exact-obligation Workboard filtering so Dashboard jumps first narrow the table to the target
