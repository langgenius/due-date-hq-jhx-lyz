...
上线，会在 Stripe billing 接入时再付一次迁移代价；该 billing 闭环已在
[`2026-04-28-pricing-stripe-billing-closure.md`](./2026-04-28-pricing-stripe-billing-closure.md)
补齐。
...
restrict`
- billing 字段（`billingCustomerId / billingSubscriptionId`）已在 Stripe billing
  闭环中补齐；RBAC 字段（`coordinatorCanSeeDollars`）/ 默认 assignee 等 P1
  字段**故意不预占**（D1 加列零成本，等真用到再 ALTER；ADR 0010 Follow-ups
  列出来了）
