...
- **不升主导航**：完整 billing center 继续留在 Organization → Billing；主导航按工作域组织，而不是按商业动作组织。
- **放 sidebar footer 轻量状态入口**：在 user menu 上方加入 `PlanStatusLink`，
  展示当前 firm 的 `Solo / Pro / Firm`、seat count，以及 owner 视角的 `Upgrade / Manage`
  或 member 视角的 `View`。点击统一进入 `/settings/billing`。
- **2026-04-29 IA follow-up**：`Import clients` 不再作为 sidebar footer 常驻 CTA；它是 clients
  activation/setup path，入口回到 `/clients`、Dashboard empty state 和 Command Palette。
