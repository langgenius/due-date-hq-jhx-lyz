...
- Added deterministic Smart Priority in `packages/core/src/priority` with PRD weights for
  ...
  Obligations, and Weekly Brief ordering now share the same score/rank/factor breakdown contract.
...
- Added `client-risk-summary@v1` and `deadline-tip@v1` prompts plus guard checks for empty
  ...
  placeholders.
- Added Client Risk Summary to the Client Profile sheet, Deadline Tip to the Obligations detail
  drawer, and Smart Priority badges/popovers to Dashboard and Obligations rows.
...
- Obligations detail `Risk` tab now shows `Preparing` state, preserves the previous tip, polls
