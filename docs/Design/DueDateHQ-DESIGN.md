...
| **SidebarContent**（nav body, flex 1） | —               | 三个 group：`MAIN`（Dashboard / Workboard，带 mono badge）`MANAGE`（`Settings` 非交互 section header：icon + label + 静态 chevron-down，无 hover bg、不可点击；下挂 `Rules` 子项 + `Members` / `Profile` `P1` placeholder 子项，缩进 ml-5、可达态 `accent-tint`）`ADMIN · P1`（Clients / Audit log / Team workload，整组 opacity 0.55，每行末尾 `P1` mono tag） |
