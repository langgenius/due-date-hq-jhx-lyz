...
| **SidebarContent**（nav body, flex 1） | —               | 三个 group：`MAIN`（Dashboard / Workboard，带 mono badge）`MANAGE`（`Settings` 非交互 section header：icon + label + 静态 chevron-down，无 hover bg、不可点击；下挂 `Profile` / `Rules` 子项 + `Members` `P1` placeholder 子项，缩进 ml-5、可达态 `accent-tint`）`ADMIN`（Clients / Audit log enabled；Team workload `P1` disabled，行末 mono tag） |
...
- Settings **forms**（Profile / Members / Org general）：max-width `880px` — 单对象编辑、字段稀疏，窄列让眼动距离短
