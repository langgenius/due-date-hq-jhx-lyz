...
| **SidebarContent**（nav body, flex 1）      | —               | 三个 group：`OPERATIONS`（Dashboard / Workboard / Alerts / Team workload）、`CLIENTS`（Clients facts）、`PRACTICE`（Practice profile / Rules / Members / Billing / Audit log）。Alerts 使用 `RadioTower`，表达正在监听的 regulatory signal / Pulse workbench；右上角 `Bell` 保留给个人 notification center。Team workload 是 paid operations surface：Solo 显示 locked `Pro` hint，Pro/Enterprise 启用。Rules 使用 `FileCheck2`，表达 verified rules / source-backed deadline templates，不使用通用 settings gear。主导航按工作心智组织，不按工程模块或权限表命名。 |
