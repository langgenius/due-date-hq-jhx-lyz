...
| **SidebarContent**（nav body, flex 1）      | —               | 三个 group：`OPERATIONS`（Dashboard / Obligations / Team workload）、`CLIENTS`（Clients facts）、`PRACTICE`（Practice profile / Rules / Members / Billing / Audit log）。Calendar sync 是 Obligations header 的二级入口（`/workboard/calendar`），不是 sidebar 一级项。Pulse Changes 合并进 Rules，Rules 使用 `FileCheck2` 并承载 verified rules、source-backed deadline templates、政府来源变更和 Pulse badge；右上角 `Bell` 保留给个人 notification center。Team workload 是 paid operations surface：Solo 显示 locked `Pro` hint，Pro/Enterprise 启用。主导航按工作心智组织，不按工程模块或权限表命名。 |
