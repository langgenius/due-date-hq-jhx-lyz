...
overview: 用 Lingui v5 给 apps/web 和 apps/server 引入中英文多语言：web 端基于浏览器语言自动检测 + localStorage 记忆 + UI 切换器完成全量文案替换；server 端仅对用户可见的邮件文案做轻量字典本地化，错误码保持稳定合约由前端映射。
todos:
  - id: deps
    content: 在 pnpm-workspace.yaml catalog 中新增 Lingui 运行时/CLI/宏/Vite 插件依赖；更新 apps/web/package.json 引用（apps/server 不动）
    status: completed
  - id: config
    content: 新增 apps/web/lingui.config.ts；修改 apps/web/vite.config.ts 接入 @vitejs/plugin-react 的 babel 宏和 @lingui/vite-plugin
    status: completed
  - id: provider
    content: 新增 apps/web/src/i18n/{i18n.ts,provider.tsx,locales.ts}；在 main.tsx 最外层包 AppI18nProvider，同步 document.documentElement.lang
    status: completed
  - id: switcher
    content: 在 _layout.tsx 的 UserMenu（panel 和 compact 两份）中新增语言切换菜单项，调用 switchLocale 并持久化
    status: completed
  - id: replace-web
    content: 逐文件替换 apps/web/src/routes/{_layout,login,dashboard,workboard,settings,error}.tsx 中所有硬编码文案为 t`` / <Trans>，运行 lingui extract 生成 en/zh-CN .po
    status: completed
  - id: intl-utils
    content: 修改 apps/web/src/lib/utils.ts 的 formatCents/formatDate 读取 i18n.locale；补充 utils.test.ts 的 zh-CN 用例
    status: completed
  - id: server-email
    content: 新增 apps/server/src/i18n/{resolve.ts,messages.ts,resolve.test.ts}；修改 auth.ts 邀请邮件 subject/html 按 locale 选取
    status: completed
  - id: rpc-header
    content: 修改 apps/web/src/lib/rpc.ts，给每个 RPC 请求带上 x-locale＝当前 i18n.locale
    status: completed
  - id: error-code-map
...
- 宏方案：走 `@lingui/babel-plugin-lingui-macro`（因为本项目 `apps/web` 用的是 `@vitejs/plugin-react` 即 Babel，非 SWC）。
- 语种：`en`（默认/兜底）、`zh-CN`。消息 id 用英文源文案自动生成（`lingui extract` 的默认 hash/original 模式）。
- 作用域（已确认）：`apps/web` 全量 + `apps/server` 邮件文案；错误码保持稳定合约由前端翻译；`packages/*` 不引入 i18n（保持 infra-free）。
...
- `apps/web/lingui.config.ts`：locales、`catalogs` 指向 `src/i18n/locales/{locale}`、format 用 `po`。
...
对应修改：
- [apps/web/package.json](apps/web/package.json)：`dependencies` 加 `@lingui/core/react/detect-locale`；`devDependencies` 加 `cli/macro/vite-plugin/babel-plugin-lingui-macro`；`scripts` 新增 `"i18n:extract": "lingui extract"`、`"i18n:compile": "lingui compile"`。
- [apps/server/package.json](apps/server/package.json)：不引入任何 Lingui 依赖（邮件使用纯字典）。

## 4. Vite 与 Lingui 接入（apps/web）
...
- [apps/web/index.html](apps/web/index.html)：`<title>` 保持 `DueDateHQ`（品牌名不译），但 `<html lang="en">` 改由 `provider.tsx` 在激活 locale 后 `document.documentElement.lang = locale` 同步。
- [apps/web/src/main.tsx](apps/web/src/main.tsx)：外层加 `<AppI18nProvider>`，包住 `QueryClientProvider`。
- [apps/web/src/routes/_layout.tsx](apps/web/src/routes/_layout.tsx)：
...
- [apps/web/src/routes/login.tsx](apps/web/src/routes/login.tsx)：`highlights` 数组改成 hook 内常量；`Phase 0 · Demo workspace`、`Verified risk, one deadline at a time.`、`Sign in`、卡片描述、`Continue with Google`、`Redirecting to Google…`、`Secure sign-in`、`Terms of Service`/`Privacy Policy`/`support@…`、footer 的 `SOC 2 ready · SSO available`。
- [apps/web/src/routes/dashboard.tsx](apps/web/src/routes/dashboard.tsx)：`pulseItems`、`queueStats`、卡片标题/描述、所有 Tabs/Table 表头、Alert 文案。注意 mock 业务数据（客户名、城市名）保留不翻译，仅翻译 UI 标签与骨架文案；`pulseItems` 的 `title/detail` 因是演示用 UI 文案故纳入翻译。
- [apps/web/src/routes/workboard.tsx](apps/web/src/routes/workboard.tsx)：标题、描述、按钮、`aria-label="Search obligations"`、`placeholder`、筛选徽章 `all/blocked/in review/waiting`、表头、`{n} rows`（用 `<Plural>`）。
- [apps/web/src/routes/settings.tsx](apps/web/src/routes/settings.tsx)：标题、字段 label/description、Select 选项（`New York` 等地区名保留英文，不翻译；但 `Multi-state` 可译）。
- [apps/web/src/routes/error.tsx](apps/web/src/routes/error.tsx)：`Route failed`、`Return to dashboard`、`Unexpected route error`。
- [apps/web/src/routes/fallback.tsx](apps/web/src/routes/fallback.tsx)：纯骨架，无文案，跳过。

## 6. 日期 / 货币本地化

改 [apps/web/src/lib/utils.ts](apps/web/src/lib/utils.ts) 的 `formatCents` / `formatDate`：
...
- 对应 test [apps/web/src/lib/utils.test.ts](apps/web/src/lib/utils.test.ts) 追加 zh-CN 分支断言。
...
web 端向 server 发请求时，在 [apps/web/src/lib/rpc.ts](apps/web/src/lib/rpc.ts) 的 fetch 层给每个请求加 `x-locale: i18n.locale`，使 server 端可以按 locale 返回邮件等本地化文案。

## 9. 测试

- [apps/web/src/lib/utils.test.ts](apps/web/src/lib/utils.test.ts)：补 zh-CN 格式化断言（先 `i18n.activate('zh-CN')`）。
- 新增 `apps/web/src/i18n/provider.test.tsx`：断言首次检测回退到 en、设置后 `localStorage` 写入、`document.documentElement.lang` 同步。
...
- `pnpm --filter @duedatehq/web i18n:compile`：编译为运行时 JS（通常 Vite 插件会自动做，CI 仍跑一遍做校验）。
